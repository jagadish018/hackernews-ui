...
export default function PastPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 401) {
            router.push(`/login?from=/posts/past-post`);
            return;
          }
  ...
  if (!session?.user) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-8 text-center">
        <p>
          Please{" "}
          <a href={`/login?from=/posts/past-post`} className="text-orange-500">
            login
          </a>{" "}
          to view past posts
        </p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pb-16">
      <h2 className="text-xl font-semibold mb-6">Past Posts</h2>
